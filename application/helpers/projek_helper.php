<?php
	...
	function tglindo($date){
		...
		return $tgl.' '.$bulanIndo[$bulan-1].' '.$tahun;
	}
	function getBulanIndo($urutan=0){
		$bulanArr = ['','Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
		return $bulanArr[$urutan];
	}
	function getWebSetting(){
		$CI = get_instance();
		$CI->load->model('InstansiProvider');
		$data = $CI->InstansiProvider->get()->row_array();
		return $data;
	}
	function getUserSetting(){
		$CI = get_instance();
		$nip=$CI->session->userdata('username');
		$CI->load->model('UserProvider');
		$data = $CI->UserProvider->get_where(['nip'=>$nip])->row_array();
		return $data;
	}
?>